...
const getStudentSchedules = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM students_schedules');
...
const createStudentSchedule = async (req, res) => {
  try {
    ...
    const [result] = await db.query('INSERT INTO students_schedules (idstudent, idschedule, status) VALUES (?, ?, ?)', [idstudent, idschedule, status]);
...
const deleteStudentSchedule = async (req, res) => {
  try {
    ...
    const [result] = await db.query('DELETE FROM students_schedules WHERE idstudent = ? AND idschedule = ?', [idstudent, idschedule]);
